...
for cell in nb['cells']:
    if cell['cell_type'] == 'code' and '1.3.A Market Saturation Forecast' in ''.join(cell['source']):
        new_source = [
            ...
            "\n",
            "# 4. Generate Monthly Future Projections (Safety Check)\n",
            ...
            "        print(f\"\\nInsight: Market saturation is projected on {saturation_point.strftime('%B %Y')}.\")\n",
            "    else:\n",
            "        print(\"\\nInsight: The model suggests market saturation has already been reached or is imminent.\")\n",
            "        df_plot = df_f[['month', 'Growth', 'Type']].rename(columns={'month': 'Month'})\n",
            "else:\n",
            "    print(\"\\nInsight: Growth is stable or accelerating (Slope >= 0). No saturation point found in the near future.\")\n",
            "    # Project 12 months ahead anyway to show the upward trend\n",
            "    future_indices = np.arange(current_idx, current_idx + 13)\n",
            "    future_months = [df_f['month'].max() + pd.DateOffset(months=int(i - current_idx + 1)) for i in future_indices]\n",
            "    future_preds = model.predict(future_indices.reshape(-1, 1))\n",
