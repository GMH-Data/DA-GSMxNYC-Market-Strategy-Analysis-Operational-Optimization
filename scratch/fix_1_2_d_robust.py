...
def fix_1_2_d_maps_robustly():
    notebook_path = 'Data.ipynb'
    with open(notebook_path, 'r', encoding='utf-8') as f:
        nb = nbf.read(f, as_version=4)

    robust_code_1_2_d = """# 1.2.D Competition Analysis: Uber vs Lyft Detailed Visualizations
...
# 2. Geometry Setup (Ensuring lowercase 'borough')
gdf = gpd.read_file('Dataset/taxi_zones/taxi_zones.shp')
gdf['LocationID'] = gdf['LocationID'].astype(int)
# Standardize casing to avoid errors
gdf.columns = [c.lower() for c in gdf.columns]

merged_gdf = gdf.merge(df_metrics, left_on='locationid', right_on='PULocationID', how='left').fillna(0)
geojson = json.loads(merged_gdf.to_json())

# --- PART 1: TWO SIMPLE GROWTH MAPS ---
# Uber Growth Map
fig_uber = px.choropleth(merged_gdf, geojson=geojson, locations='locationid', featureidkey='properties.locationid',
...
# Lyft Growth Map
fig_lyft = px.choropleth(merged_gdf, geojson=geojson, locations='locationid', featureidkey='properties.locationid',
...
fig_comp = px.choropleth(merged_gdf, geojson=geojson, locations='locationid', featureidkey='properties.locationid',
...
fig_comp.show()"""
    ...
    for cell in nb.cells:
        if cell.cell_type == 'code' and '# 1.2.D' in cell.source:
            cell.source = robust_code_1_2_d
            break
            
    with open(notebook_path, 'w', encoding='utf-8') as f:
        nbf.write(nb, f)
    print("Section 1.2.D updated with robust ID-based mapping and column normalization.")

if __name__ == "__main__":
    fix_1_2_d_maps_robustly()
