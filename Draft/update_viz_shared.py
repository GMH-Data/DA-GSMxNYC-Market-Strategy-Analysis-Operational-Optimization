...
def update_visualization_shared_axes(nb_path):
    # Load the notebook
    with open(nb_path, 'r', encoding='utf-8') as f:
        nb = nbf.read(f, as_version=4)

    # Updated code with Shared Axes for 2x2 Facet Plots
...
# Visualization: 2x2 Facet Plot with SHARED AXES
fig_corr = px.scatter(df_corr_val, x='trips', y='revenue', color='provider', 
                      facet_col='provider', facet_col_wrap=2,
                      trendline='ols', title='Correlation: Trip Volume vs Revenue (Shared Scales Comparison)',
...
# Force shared axes to make them comparable
fig_corr.update_xaxes(matches='x', showticklabels=True)
fig_corr.update_yaxes(matches='y', showticklabels=True)

# Cleanup subplot titles
fig_corr.for_each_annotation(lambda a: a.update(text=a.text.split(\"=\")[-1]))
fig_corr.update_layout(plot_bgcolor=THEME['bg'], paper_bgcolor=THEME['bg'], height=800, showlegend=False)
fig_corr.show()
...
print(\"\\\\nCONCLUSION: High positive correlation (close to 1.0) justifies using Trip Volume as a primary metric for Market Share.\")"""
    ...
    if updated:
        with open(nb_path, 'w', encoding='utf-8') as f:
            nbf.write(nb, f)
        print("Successfully updated the correlation cell with Shared Axes.")
    else:
        print("Cell not found.")

if __name__ == "__main__":
    path = "Analysis.ipynb"
    update_visualization_shared_axes(path)
