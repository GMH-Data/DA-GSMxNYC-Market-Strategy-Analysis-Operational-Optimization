...
def add_correlation_cell(nb_path):
    # Load the notebook
    with open(nb_path, 'r', encoding='utf-8') as f:
        nb = nbf.read(f, as_version=4)

    # Define the new code cell
    new_code = """# [2.2.1] Correlation between Trip Volume and Revenue
...
print(\"\\\\nCONCLUSION: High positive correlation (close to 1.0) justifies using Trip Volume as a primary metric for Market Share.\")"""
    ...
    new_cell.id = "validation_trips_revenue_corr"

    # Find the target location: After 2.2.1 markdown cell
    target_index = -1
    for i, cell in enumerate(nb.cells):
        if cell.cell_type == 'markdown' and '### 2.2.1.' in cell.source:
            target_index = i + 1
            break

    if target_index != -1:
        # Check if already exists to avoid duplication
        exists = any(cell.id == "validation_trips_revenue_corr" for cell in nb.cells)
        if not exists:
            nb.cells.insert(target_index, new_cell)
            print(f"Successfully inserted cell at index {target_index}")
        else:
            print("Cell already exists.")
    else:
        print("Target markdown cell 2.2.1 not found.")
